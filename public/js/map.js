...
function initMap() {
	...
	//
	// }
    $('.contact_map').each(function(index, value){
        var _this = $(value);
        var map = new google.maps.Map(document.getElementById('map_' + _this.data('id')), {
            center: {lat: 56.0080264, lng: 37.4422701},
            scrollwheel: false,
            scaleControl: false,
            zoom: 16
        ...
        var contentString = '<div class="map_tooltip">'+
            '<div class="inner">'+
            '<h3>' + _this.data('name') + '</h3>'+
            '<ul class="info">'+
            '<li><i class="map-mark_2"></i>' + _this.data('address') + '</li>'+
            '</ul>'+
            '</div>'+
            '</div>';
